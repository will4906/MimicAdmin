...
def find_min_list_first(result_list):
    min_value = 999999999
    min_second = 0
        ...
        if p[3] is not None and p[3] != '':
            try:
                ...
                if max_fio2 is not None:
                    ...
                    result_list.append((result, float(max_fio2)))
            except:
                print(p)

    if len(result_list) > 0:
        pao2_fio2_list.append((ards[0], find_min_list_first(result_list)))

for pf in pao2_fio2_list:
    print('''
        UPDATE ards_whole SET pfio2 = %s WHERE hadm_id = %s;
        '''% (pf[1][1], pf[0]))
    cur.execute(
        '''
        UPDATE %s SET pfio2 = %s WHERE hadm_id = %s;
        '''% (table_name, pf[1][1], pf[0])
    )
    conn.commit()
conn.close()
