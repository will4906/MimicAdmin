...
cursor.execute(
    '''
    SELECT icustays.icustay_id, icustays.intime, icustays.outtime FROM icustays, head_whole WHERE head_whole.icustay_id = icustays.icustay_id;
    '''
)
...
for icu_info in icu_info_list:
    ...
    # 7day
    if outtime - intime > timedelta(days=2):
        outtime = intime + timedelta(days=2)
    ...
    cursor.execute(
        "INSERT INTO head_uo_48hr(icustay_id) VALUES (%s);", (icustay_id,)
    )
    conn.commit()

    if min_6 != 1000000:
        cursor.execute(
            "UPDATE head_uo_48hr SET uo_6hr = %s WHERE icustay_id = %s;", (min_6, icustay_id)
        )
        conn.commit()

    if min_12 != 1000000:
        cursor.execute(
            "UPDATE head_uo_48hr SET uo_12hr = %s WHERE icustay_id = %s;", (min_12, icustay_id)
        )
        conn.commit()

    if min_24 != 1000000:
        cursor.execute(
            "UPDATE head_uo_48hr SET uo_24hr = %s WHERE icustay_id = %s;", (min_24, icustay_id)
        )
        conn.commit()
        ...
        if kdigo[2] is not None and kdigo[5] is not None:
            uo_24hr = kdigo[5] / kdigo[2] / 24
            cursor.execute(
                "UPDATE head_uo_48hr SET uo_24hr = %s WHERE icustay_id = %s;", (uo_24hr, icustay_id)
            )
            conn.commit()
