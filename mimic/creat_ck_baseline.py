...
if __name__ == '__main__':
    ...
    for patient in patients:
        ...
        if STANDARD == 'akin':
            ...
            stage = ck(min_value, max_value)
        # print(hadm_id, 'kdigo:', kdigo_stage)
        print(hadm_id, STANDARD, stage)

        cursor.execute("SELECT stage_ck_creat_by_base FROM {} WHERE hadm_id = {};".format(TABLE_NAME, hadm_id))
        old = cursor.fetchone()[0]
        if old is None:
            cursor.execute("UPDATE {} SET stage_ck_creat_by_base = {} WHERE hadm_id = {};".format(TABLE_NAME, stage, hadm_id))
        elif stage is None:
            pass
        elif int(old) < int(stage):
            cursor.execute("UPDATE {} SET stage_ck_creat_by_base = {} WHERE hadm_id = {};".format(TABLE_NAME, stage, hadm_id))
        else:
            pass
