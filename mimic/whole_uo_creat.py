...
def init_field(conn, cursor):
    def add_field(field):
        try:
            ...
            conn.rollback()
    add_field('stage_kdigo_by_min')
    add_field('stage_rifie_by_min')
    add_field('stage_kdigo_by_min_without_limit')
    add_field('stage_rifie_by_min_without_limit')
...
if __name__ == '__main__':
    ...
    cursor.execute("UPDATE {} SET stage_kdigo_by_min = NULL;".format(TABLE_NAME))
    conn.commit()
    cursor.execute("UPDATE {} SET stage_kdigo_by_min = stage_kdigo_creat_by_min;".format(TABLE_NAME))
    conn.commit()
    cursor.execute('''
        UPDATE {} SET stage_kdigo_by_min = stage_kdigo_7day_admin_uo WHERE stage_kdigo_7day_admin_uo IS NOT NULL
        AND stage_kdigo_7day_admin_uo > stage_kdigo_creat_by_min AND stage_kdigo_creat_by_min >= 1;
        '''.format(TABLE_NAME))
    conn.commit()
    ...
    """
    # 算kdigo无限制
    # cursor.execute("UPDATE {} SET stage_kdigo_by_min_without_limit = NULL;".format(TABLE_NAME))
    # conn.commit()
    # cursor.execute("UPDATE {} SET stage_kdigo_by_min_without_limit = stage_kdigo_creat_by_min;".format(TABLE_NAME))
    # conn.commit()
    # cursor.execute('''
    # UPDATE {} SET stage_kdigo_by_min_without_limit = stage_kdigo_7day_admin_uo WHERE stage_kdigo_7day_admin_uo IS NOT NULL
    # AND stage_kdigo_7day_admin_uo > stage_kdigo_creat_by_min;
    # '''.format(TABLE_NAME))
    # conn.commit()
